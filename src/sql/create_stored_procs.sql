...
INSERT INTO [bca].[scenario_comparison] (
	[analysis_id]
	,[scenario_year]
	,[scenario_id_base]
	,[scenario_id_build]
...
IF @scenario_year <> @ref_year
	...
	-- Emissions cost calculator
		...
		AND 
			[scenario_comparison].[scenario_year] = @scenario_year;
	END
...
IF @scenario_year <> @ref_year
	...
	-- Aggregate toll calculator
	    ...
	    FROM
			[bca].[scenario_comparison]
			CROSS JOIN [bca].[fn_aggregate_toll](
				@base_scenario_id
				,@build_scenario_id)
	    WHERE
			[scenario_comparison].[analysis_id] = @analysis_id
	...
	-- Aggregate trips calculator
		...
		FROM
			[bca].[scenario_comparison]
			CROSS JOIN [bca].[fn_aggregate_trips](
				@base_scenario_id
				,@build_scenario_id
				,@vot_ctm
				,@vot_truck)
	...
	-- Resident person trips AT
		...
		FROM
			[bca].[scenario_comparison]
			CROSS JOIN [bca].[fn_resident_trips_at](
				@base_scenario_id
				,@build_scenario_id
				,@vot_commute
				,@vot_noncommute)
	...
	-- Resident person trips auto
		...
		FROM
			[bca].[scenario_comparison]
			CROSS JOIN [bca].[fn_resident_trips_auto](
				@base_scenario_id
				,@build_scenario_id
				,@vot_commute
				,@vot_noncommute)
